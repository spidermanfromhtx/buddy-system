...
function Incoming() {
  ...
  async function answer() {
    stopRing(); const camera = allowCamera && useCam;
    try { await getLocalStream(camera, dummy); } catch { /* Call screen shows Join the line if the mic is still blocked. */ }
    void unlockOutput();
    void nav({ to: "/call/$id", params: { id }, search: { ...search, camera, allowCamera, room: search.room ?? q.data?.room } });
  }
  ...
  return (
    <main className="flex min-h-dvh flex-col bg-night text-paper">
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center px-6 py-12 text-center">
        <p className="text-xs uppercase tracking-[0.22em] text-paper/50">{dummy ? "demo ring" : ringingYou || bothRing ? "incoming" : "calling"}</p>
        <div className="mt-10"><Face name={name} color={color} photo={photo} size="lg" ringing /></div>
        <h1 className="mt-8 font-display text-5xl tracking-tight">{name}</h1>
        <p className="mt-3 text-lg text-paper/80">{task}</p>
        <p className="mt-2 text-sm text-paper/50">{urgent ? "urgent · " : ""}{lengthMin} min{allowCamera ? " · camera available" : ""}</p>
        {allowCamera ? <div className="mt-8 flex w-full gap-2"><Btn type="button" kind={!useCam ? "fill" : "night"} className="flex-1" onClick={() => setUseCam(false)}>Audio only</Btn><Btn type="button" kind={useCam ? "fill" : "night"} className="flex-1" onClick={() => setUseCam(true)}>Camera</Btn></div> : null}
        <div className="mt-12 flex w-full flex-col gap-2"><Btn kind="fill" className="h-14 w-full text-lg" onClick={() => void answer()}>Answer</Btn><Btn kind="night" className="h-12 w-full" onClick={() => void decline()}>Decline</Btn></div>
      </div>
    </main>
  );
}
