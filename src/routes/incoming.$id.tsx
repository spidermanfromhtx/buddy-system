...
function Incoming() {
  ...
  const other =
    me && q.data?.callerId === me.id
      ? { name: q.data.calleeName, color: q.data.calleeColor, photo: q.data.calleePhoto }
      : me && q.data?.calleeId === me.id
        ? { name: q.data.callerName, color: q.data.callerColor, photo: q.data.callerPhoto }
        : null;
  ...
  useEffect(() => {
    if (dummy) return;
    if (q.data?.status === "declined" || q.data?.status === "done") {
      stopRing();
      void nav({ to: "/feed" });
    }
  ...
  async function decline() {
    stopRing();
    if (!dummy) await setCallStatus({ data: { id, status: "declined" } });
    void nav({ to: "/feed" });
  }
