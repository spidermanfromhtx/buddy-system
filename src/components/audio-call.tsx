...
export function AudioCall({ room, selfId, name, wantCamera, allowCamera = false, onCamera, loopback = false }: {
  ...
  useEffect(() => {
    if (!allowCamera || lastCam.current === wantCamera) return;
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      {allowCamera && (status === "connected" || status === "waiting") ? <div className="flex w-full gap-2"><Btn type="button" kind={!wantCamera ? "ink" : "line"} className="flex-1" onClick={() => onCamera?.(false)}>Camera off</Btn><Btn type="button" kind={wantCamera ? "ink" : "line"} className="flex-1" onClick={() => onCamera?.(true)}>Camera on</Btn></div> : null}
