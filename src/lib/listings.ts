...
export const claimBooking = createServerFn({ method: "POST" }).validator((d: unknown) => z.object({ listingId: ID, peerId: ID, name: z.string().min(1).max(40), color: z.string().max(16), photo: PHOTO }).parse(d)).handler(async ({ data }) => {
  ...
  await sql.query(`UPDATE listings SET match_id = $1, match_peer_id = $2, match_peer_name = $3, match_peer_color = $4, match_peer_photo = $5, match_room = $6, match_call_id = $7 WHERE id = $8`, [matchId, data.peerId, data.name, data.color, data.photo ?? null, room, callId, target.id]);
  await sql.query(`UPDATE listings SET match_id = $1, match_peer_id = $2, match_peer_name = $3, match_peer_color = $4, match_peer_photo = $5, match_room = $6, match_call_id = $7 WHERE id = $8`, [matchId, target.peer_id, target.name, target.color, target.photo ?? null, room, callId, myId]);
