...
export type Listing = { id: string; peerId: string; name: string; color: string; photo: string | null; task: string; urgent: boolean; mode: "live" | "scheduled"; lengthMin: number; camera: boolean; similarPref: string; windowLabel: string | null; windowStart: string | null; windowEnd: string | null; dueDate: string | null; expiresAt: string | null; school: string | null; matchId: string | null; matchPeerId: string | null; matchPeerName: string | null; matchRoom: string | null; matchCallId: string | null; category: string | null; ratingAvg: number | null; ratingCount: number; };
function mapRow(r: Record<string, unknown>): Listing { return { id: String(r.id), peerId: String(r.peer_id), name: String(r.name), color: String(r.color), photo: r.photo ? String(r.photo) : null, task: String(r.task), urgent: Boolean(r.urgent), mode: r.mode === "scheduled" ? "scheduled" : "live", lengthMin: Number(r.length_min ?? 25), camera: Boolean(r.camera), similarPref: String(r.similar_pref ?? "either"), windowLabel: r.window_label ? String(r.window_label) : null, windowStart: r.window_start ? String(r.window_start) : null, windowEnd: r.window_end ? String(r.window_end) : null, dueDate: r.due_date ? String(r.due_date) : null, expiresAt: r.expires_at ? String(r.expires_at) : null, school: r.school ? String(r.school) : null, matchId: r.match_id ? String(r.match_id) : null, matchPeerId: r.match_peer_id ? String(r.match_peer_id) : null, matchPeerName: r.match_peer_name ? String(r.match_peer_name) : null, matchRoom: r.match_room ? String(r.match_room) : null, matchCallId: r.match_call_id ? String(r.match_call_id) : null, category: r.category ? String(r.category) : null, ratingAvg: r.rating_avg != null ? Number(r.rating_avg) : null, ratingCount: r.rating_n != null ? Number(r.rating_n) : 0 }; }

export const listOpen = createServerFn({ method: "GET" }).validator((d: unknown) => z.object({ tab: z.enum(["all", "school"]).optional(), peerId: ID.optional(), token: z.string().max(128).optional() }).parse(d ?? {})).handler(async ({ data }): Promise<Listing[]> => { try { const sql = await getSql(); await activateDueMatches(sql); if (data.tab === "school") { if (!data.peerId || !data.token) return []; const member = await sql.query(`SELECT school FROM campus_members WHERE peer_id = $1 AND token = $2 LIMIT 1`, [data.peerId, data.token]); const school = member[0]?.school ? String(member[0].school) : null; if (!school) return []; const rows = await sql.query(`SELECT l.*, s.avg as rating_avg, s.n as rating_n FROM listings l LEFT JOIN (SELECT subject_id, avg(rating)::float as avg, count(*)::int as n FROM reviews GROUP BY subject_id) s ON s.subject_id = l.peer_id WHERE school = $1 AND (expires_at IS NULL OR expires_at > now()) AND peer_id NOT LIKE 'dummy-%' AND peer_id NOT IN (SELECT id FROM accounts WHERE banned = true) ORDER BY urgent DESC, created_at DESC LIMIT 80`, [school]); return rows.map(mapRow); } const rows = await sql.query(`SELECT l.*, s.avg as rating_avg, s.n as rating_n FROM listings l LEFT JOIN (SELECT subject_id, avg(rating)::float as avg, count(*)::int as n FROM reviews GROUP BY subject_id) s ON s.subject_id = l.peer_id WHERE (school IS NULL OR school = '') AND (expires_at IS NULL OR expires_at > now()) AND peer_id NOT LIKE 'dummy-%' AND peer_id NOT IN (SELECT id FROM accounts WHERE banned = true) ORDER BY urgent DESC, created_at DESC LIMIT 80`); return rows.map(mapRow); } catch { return []; } });
...
export const claimBooking = createServerFn({ method: "POST" }).validator((d: unknown) => z.object({ listingId: ID, peerId: ID, name: z.string().min(1).max(40), color: z.string().max(16), photo: PHOTO }).parse(d)).handler(async ({ data }) => {
  ...
  const myId = sid("book");
  await sql.query(
    `INSERT INTO listings (id, peer_id, name, color, photo, task, urgent, mode, length_min, camera, similar_pref, window_label, window_start, window_end, due_date, school, category, expires_at) VALUES ($1,$2,$3,$4,$5,$6,$7,'scheduled',$8,$9,$10,$11,$12,$13,$14,$15,$16, $13::timestamptz)`,
    [
      myId,
      data.peerId,
      data.name,
  ...
  await sql.query(`UPDATE listings SET match_id = $1, match_peer_id = $2, match_peer_name = $3, match_room = $4, match_call_id = $5 WHERE id = $6`, [matchId, data.peerId, data.name, room, callId, target.id]);
  await sql.query(`UPDATE listings SET match_id = $1, match_peer_id = $2, match_peer_name = $3, match_room = $4, match_call_id = $5 WHERE id = $6`, [matchId, target.peer_id, target.name, room, callId, myId]);
