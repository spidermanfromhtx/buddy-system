...
export const claimBooking = createServerFn({ method: "POST" }).validator((d: unknown) => z.object({ listingId: ID, peerId: ID, name: z.string().min(1).max(40), color: z.string().max(16), photo: PHOTO }).parse(d)).handler(async ({ data }) => {
  ...
  const hostName = String(target.host_name ?? target.name);
  const hostColor = String(target.host_color ?? target.color ?? "");
  const hostPhoto = target.host_photo ? String(target.host_photo) : target.photo ? String(target.photo) : null;
  await sql.query(
